...
public class JavaDevVmTest {
    ...
    void testLocalTemplates() throws IOException, InterruptedException {
        commandExecutor.assertPathExistsAndContains("/etc/jdvm-templates/local", ".aliases.local", ".bash_logout.local",
                ".bashrc.local", ".env.local", ".profile.local", ".zlogout.local", ".zprofile.local", ".zshenv.local",
                ".zshrc.local");
        commandExecutor.assertPathFilesAndDirsCount("/etc/jdvm-templates/local", 9);
    }

    @Test
    void testUserTemplates() throws IOException, InterruptedException {
        commandExecutor.assertPathExistsAndContains("/etc/jdvm-templates/user", ".aliases", ".bash_aliases", ".env", ".ps1",
                ".zlogout", ".zprofile", ".zshenv", ".zshrc");
        commandExecutor.assertPathFilesAndDirsCount("/etc/jdvm-templates/user", 8);
    }
