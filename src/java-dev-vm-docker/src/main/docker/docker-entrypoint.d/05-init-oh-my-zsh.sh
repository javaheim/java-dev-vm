...
linkGitStatus() {
  if [[ ! -e "/home/${USER}/.cache/gitstatus/gitstatusd-linux-x86_64" ]]; then
    ...
    ln -v --symbolic "/opt/oh-my-zsh/themes/powerlevel10k/gitstatus/gitstatusd-linux-x86_64" "/home/${USER}/.cache/gitstatus/gitstatusd-linux-x86_64"
  fi
}
...
fixOhMyZshInIntelliJIdea() {
  ...
  if [[ -e "${intellijIdeaFontsDir}" ]]; then
    if [[ ! -e "${intellijIdeaFontsDir}/MesloLGS-NF-Regular.ttf" ]]; then
      ...
      ln -v --symbolic "${fontsDir}/meslo/MesloLGS-NF-Regular.ttf" "${intellijIdeaFontsDir}/MesloLGS-NF-Regular.ttf"
    fi
    if [[ ! -e "${intellijIdeaFontsDir}/MesloLGS-NF-Bold.ttf" ]]; then
      echo -e "${INFO} Fix OhMyZsh in IntelliJ IDEA: MesloLGS-NF-Bold.ttf..."
      ln -v --symbolic "${fontsDir}/meslo/MesloLGS-NF-Bold.ttf" "${intellijIdeaFontsDir}/MesloLGS-NF-Bold.ttf"
    fi
    if [[ ! -e "${intellijIdeaFontsDir}/MesloLGS-NF-Italic.ttf" ]]; then
      echo -e "${INFO} Fix OhMyZsh in IntelliJ IDEA: MesloLGS-NF-Italic.ttf..."
      ln -v --symbolic "${fontsDir}/meslo/MesloLGS-NF-Italic.ttf" "${intellijIdeaFontsDir}/MesloLGS-NF-Italic.ttf"
    fi
    if [[ ! -e "${intellijIdeaFontsDir}/MesloLGS-NF-Bold-Italic.ttf" ]]; then
      ...
      ln -v --symbolic "${fontsDir}/meslo/MesloLGS-NF-Bold-Italic.ttf" "${intellijIdeaFontsDir}/MesloLGS-NF-Bold-Italic.ttf"
    fi
  fi
}
