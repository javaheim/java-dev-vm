...
setupCorepack() {
  if [[ ! -e "/home/${USER}/.cache/node/corepack" ]]; then
    echo -e "${INFO} Setup Corepack"
    if [[ ! -e "/home/${USER}/.cache/node" ]]; then
      mkdir -v --parents "/home/${USER}/.cache/node"
    fi
    ln -v --symbolic "/opt/node/.cache/corepack" "/home/${USER}/.cache/node/corepack"
  fi
}
...
setupFirefox() {
  ...
  if [[ ! -e "${profileDir}/user.js" ]]; then
    ...
    ln -v --symbolic "/etc/jdvm-config/betterfox-user.js" "${profileDir}/user.js"
  else
...
setupGedit() {
  if [[ ! -e "/home/${USER}/.config/dconf/user" ]]; then
    ...
    ln -v --symbolic "/etc/jdvm-templates/base/dconf/user" "/home/${USER}/.config/dconf/user"
  fi
}
...
setupKitty() {
  if [[ ! -e "/home/${USER}/.config/kitty/kitty.conf" ]]; then
    ...
    ln -v --symbolic "/etc/jdvm-config/kitty.conf" "/home/${USER}/.config/kitty/kitty.conf"
  fi
}
...
setupKubectlKrew() {
  if [[ ! -e "/home/${USER}/.krew/index" ]]; then
    ...
    ln -v --symbolic "/opt/krew/index" "/home/${USER}/.krew/index"
  fi
  shopt -s nullglob
...
setupLauncher() {
  ...
  shopt -s nullglob
  for app in "/etc/jdvm-launcher/config/apps"/*.app; do
    if [[ ! -e "/home/${USER}/.jdvm-launcher/$(basename "${app}")" ]]; then
      echo -e "${INFO} Setup Launcher: $(basename "${app}")..."
      ln -v --symbolic "${app}" "/home/${USER}/.jdvm-launcher/$(basename "${app}")"
    fi
  done
  shopt -u nullglob
}
...
setupSdkMan() {
  ...
  for dir in "/opt/sdkman"/*; do
    if [[ ! -e "/home/${USER}/.sdkman/$(basename "${dir}")" ]]; then
      ...
      ln -v --symbolic "${dir}" "/home/${USER}/.sdkman/$(basename "${dir}")"
    fi
  done
  shopt -u nullglob
}
...
setupTealdeer() {
  if [[ ! -e "/home/${USER}/.config/tealdeer/config.toml" ]]; then
    ...
    ln -v --symbolic "/etc/jdvm-config/tealdeer-config.toml" "/home/${USER}/.config/tealdeer/config.toml"
  fi
}
